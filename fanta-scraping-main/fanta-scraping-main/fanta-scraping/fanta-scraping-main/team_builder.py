...
def val_partite(i): # [0, 7]
    ...
    # for players not playing previous seasons, provide a prediction based on current (this season) data
    titolarità_prediction = (partite_0 / giornate_correnti) * 38

    if(partite_1 == 0 and partite_2 == 0):
        v_partite = ((gamma * partite_0 * partite_0 / giornate_correnti) + (alpha * titolarità_prediction * titolarità_prediction / 38) + (beta * titolarità_prediction * titolarità_prediction / 38)) / 10
    elif partite_1 == 0:
        v_partite = ((gamma * partite_0 * partite_0 / giornate_correnti) + (alpha * titolarità_prediction * titolarità_prediction / 38) + (beta * partite_2 * partite_2 / 38)) / 10
    elif partite_2 == 0:
        v_partite = ((gamma * partite_0 * partite_0 / giornate_correnti) + (alpha * partite_1 * partite_1 / 38) + (beta * titolarità_prediction * titolarità_prediction / 38)) / 10
    else:
...
def val_fm(i): # [0, 15]
    ...
    titolarità_prediction = (partite_0 / giornate_correnti)
    fm_prediction = fantam_0 * titolarità_prediction / 38
    
    if(partite_1 == 0 and partite_2 == 0):
        v_fantam = (gamma * fantam_0 * partite_0 / giornate_correnti) + (alpha * fm_prediction) + (beta * fm_prediction)
    elif partite_1 == 0:
        v_fantam = (gamma * fantam_0 * partite_0 / giornate_correnti) + (alpha * fm_prediction) + (beta * fantam_2 * partite_2 / 38)
    elif partite_2 == 0:
        v_fantam = (gamma * fantam_0 * partite_0 / giornate_correnti) + (alpha * fantam_1 * partite_1 / 38) + (beta * fm_prediction)
    else:
...
def val_mv(i): # [0, 11]
    ...
    titolarità_prediction = (partite_0 / giornate_correnti)
    mv_prediction = mediav_0 * titolarità_prediction / 38
    
    if(partite_1 == 0 and partite_2 == 0):
        v_mediav = (gamma * mediav_0 * partite_0 / giornate_correnti) + (alpha * mv_prediction) + (beta * mv_prediction)
    elif partite_1 == 0:
        v_mediav = (gamma * mediav_0 * partite_0 / giornate_correnti) + (alpha * mv_prediction) + (beta * mediav_2 * partite_2 / 38)
    elif partite_2 == 0:
        v_mediav = (gamma * mediav_0 * partite_0 / giornate_correnti) + (alpha * mediav_1 * partite_1 / 38) + (beta * mv_prediction)
    else:
        v_mediav = (gamma * mediav_0 * partite_0 / giornate_correnti) + (alpha * mediav_1 * partite_1 / 38) + (beta * mediav_2 * partite_2 / 38)

    return v_mediav

def val_rigori(i): # [0, 10]
...
def val(i): # [0, 42]
    ...
    v_mv = val_mv(i)
    v_r = val_rigori(i)
    v_rp = val_rigori_parati(i)
    v_amm = val_amm(i)
    v_esp = val_esp(i)
    v_s = val_squadra(i)
    v_gs = val_gol_subiti(i)

    val = v_par + v_fm + v_mv + v_r + v_rp + v_amm + v_esp + v_s + v_gs
    
    valori['id'].append(i)
    valori['val'].append(val)
    valori['v_par'].append(v_par)
    valori['v_fm'].append(v_fm)
    valori['v_mv'].append(v_mv)
    valori['v_r'].append(v_r)
    valori['v_rp'].append(v_rp)
    valori['v_amm'].append(v_amm)
    valori['v_esp'].append(v_esp)
    valori['v_s'].append(v_s)
    valori['v_gs'].append(v_gs)

    df = pd.DataFrame({'Id':valori['id'],
                    'Valore':valori['val'],
                    'V_Partite':valori['v_par'],
                    'V_FM':valori['v_fm'],
                    'V_MV':valori['v_mv'],
                    'V_Rigori':valori['v_r'],
                    'V_Rig_Parati':valori['v_rp'],
                    'V_Ammonizioni':valori['v_amm'],
                    'V_Espulsioni':valori['v_esp'],
                    'V_Squadra':valori['v_s'],
                    'V_Gol_Subiti':valori['v_gs']})
    if i == len(df_0):
        filename = 'valori.csv'
        df.to_csv(filename)
